...
create or replace view pagestream as 
    select id, owner_id, name, path, status, created_at, updated_at from private.pagestream;
grant select, insert, update on pagestream to external_user;
grant select, update on pagestream to internal_worker;

create or replace view file as 
    select id, owner_id, pagestream_id, path, from_page, to_page, name from private.file;
